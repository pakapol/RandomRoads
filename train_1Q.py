...
def run(env, Q, ep_cap=1000, testing=False):
  ...
  while False in done and num_steps < ep_cap:
    a1 = Q.get_action(tuple(s1), testing=testing)
    a2 = Q.get_action(tuple(s2), testing=testing)
    (s1p,s2p),(r1,r2),done = env.step(np.array([a1,a2]))
    Q.record(tuple(s1),a1,r1,tuple(s1p))
    Q.record(tuple(s2),a2,r2,tuple(s2p))
    s1, s2 = s1p, s2p
    num_steps += 1
  if testing:
...
def main():
  Q = SimpleQAgent(nS=None, nA=config.num_actions, gamma=config.gamma, lr=config.lr, epsilon=config.epsilon)
